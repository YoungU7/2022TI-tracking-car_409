...
#define KP 9.0 //位置PID比例参数KP 1.2
#define KD 0.8 //位置PID微分参数KD 1.0
...
#define standard_speed 0.7//标准速度，可设置为变量通过按钮更改
...
void loop() {
      if(millis()>15000&&gray_sensor==0x0E)
  {
...
{/*
  ...
  */
  ...
  switch(gray_sensor)
  {
    ...
    case 0x08:return -9;//01000
    case 0x14:return 0;//10100
    case 0x0C:return -4;//01100
    case 0x04:return 0;//00100
    case 0x06:return 4;//00110
    case 0x02:return 9;//00010
...
//  pwm=constrain(KP*sensor_err+(sensor_err-sensor_last_err)*KD,(int)(-standard_speed*255*0.05),(int)(standard_speed*255*0.05));//约束pwm不超过标准速度的±5%
  ...
  speed_l_l = speed_l+(speed_l_err*kpl)+(speed_l_err+speed_l_last_err)*kil+(speed_l_err-speed_l_last_err)*kdl;//实际左轮加权速度
  ...
  speed_r_r = speed_r+(speed_r_err*kpr)+(speed_r_err+speed_r_last_err)*kir+(speed_r_err-speed_r_last_err)*kdr;//实际右轮加权速度
  
    if(millis()>5000&&gray_sensor==0x0E)
  {
    speed_l_l=speed_r_r=0;
    Serial1.write('s');
  }
...
{ 
  ...
  { 
    time1 = time2;
    speed_l = ((((motor_l-last_motor_l)/1040.)*0.22)/0.02); //车轮周长20cm，车轮一圈脉冲四倍频计数2496，（脉冲差/2496*0.2m）/（20ms=0.02）=speed  
    speed_r = ((((motor_r-last_motor_r)/1040.)*0.22)/0.02);//车轮周长20cm，车轮一圈脉冲四倍频计数2496，（脉冲差/2496*0.2m）/（20ms=0.02）=speed 
